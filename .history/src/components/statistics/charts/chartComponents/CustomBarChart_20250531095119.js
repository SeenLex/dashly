...
function CustomTooltip({
  ...
  categoryKey,
  teamsByCategory = {},
  slaStatusByCategory = [],  // array de obiecte SLA cu "name" = label
  isPercent = false,
}) {
  if (!(active && payload && payload.length)) return null;

  // Extrage datele principale din payload (pentru un singur bar sau multiple)
  // payload e un array, de ex pt stacked bars
  // payload[i].dataKey e cheia (ex: "count", "perc", "Met", "Exceeded")
  // payload[i].value e valoarea

  // Formatează valorile pentru afișare
  ...
  // Echipele aferente categoriei (dacă există)
  const teams = teamsByCategory[label] || [];

  // SLA status pentru categoria curentă (dacă există)
  const slaStatus = slaStatusByCategory.find((item) => item.name === label);

  return (
    <div style={{ padding: 12, backgroundColor: "#fff", border: "1px solid #ccc", borderRadius: 4, maxWidth: 300 }}>
