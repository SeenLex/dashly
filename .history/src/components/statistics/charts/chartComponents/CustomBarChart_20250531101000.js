...
function CustomTooltip({
  ...
  // Get teams and projects for this category
  const teams = getTeamsForCategory ? getTeamsForCategory(label) : [];
  const projects = getProjectsForCategory ? getProjectsForCategory(label) : [];
  ...
  return (
    <div style={{ 
      ...
      {teams.length > 0 && (
        <>
          ...
          <p style={{ margin: "6px 0", fontWeight: "bold" }}>
            {teams.length === 1 ? 'Team' : 'Teams'} working on this {categoryKey}:
          </p>
      ...
      {/* Projects section */}
      {projects.length > 0 && chartType !== 'project' && (
        <>
          <hr style={{ margin: '8px 0', borderColor: '#eee' }} />
          <p style={{ margin: "6px 0", fontWeight: "bold" }}>
            {projects.length === 1 ? 'Project' : 'Projects'} in this {categoryKey}:
          </p>
          <ul style={{ paddingLeft: 18, margin: 0 }}>
            {projects.map((project) => (
              <li key={project}>
                Project {project}
                {/* Show teams for each project if available */}
                {getTeamsForProject && (
                  <div style={{ marginLeft: '10px', fontSize: '0.9em' }}>
                    <em>Teams: {getTeamsForProject(project).join(', ') || 'None'}</em>
                  </div>
                )}
              </li>
            ))}
...
const getTeamsForCategory = (category) => {
  return teamsByCategory[categoryKey]?.[category] || [];
};

const getProjectsForCategory = (category) => {
  if (categoryKey === 'project') return [category];
