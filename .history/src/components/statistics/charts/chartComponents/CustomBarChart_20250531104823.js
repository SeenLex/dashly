...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div style={{ 
      ...
      borderRadius: 4,
      fontSize: 14,
      maxWidth: 500,       // lățime maximă mărită (în loc de 300)
      width: 500,          // lățime fixă (poți să ajustezi sau să elimini dacă vrei responsive)
      maxHeight: 400,      // înălțime maximă mărită (în loc de 200)
      overflowY: "auto",
    }}>
      ...
      <ul style={{ paddingLeft: 20, margin: 0 }}>
        {tickets.map(ticket => (
          <li key={ticket.id} style={{ marginBottom: 6 }}>
            ...
            <div><strong>Project:</strong> {ticket.project || "N/A"}</div>
            <div><strong>Team:</strong> {ticket.team_assigned_person || "N/A"}</div>
          </li>
        ))}
      </ul>
    </div>
