...
function CustomTooltip({ active, payload, label, showPercentage }) {
  ...
  const handleWheel = (e) => {
    ...
    e.stopPropagation();
  };  const x = payload[0].coordinate?.x || 0;
  const y = payload[0].coordinate?.y || 0;

  return (
    <div    style={{
      position: "fixed",             // sau "absolute", în funcție de cum setezi poziția
     top: y + 10,      // mic offset ca să nu acopere cursorul
    left: x + 10,  
      zIndex: 9999,                 // mare, ca să fie peste alte elemente
