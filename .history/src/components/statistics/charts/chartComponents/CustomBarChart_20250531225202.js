...
function CustomTooltip({ active, payload, label, showPercentage,coordinate }) {
 ...
 const tooltipWidth = 550; // cum ai în stil
  const margin = 20;        // un mic spațiu față de margine

  // Pozițiile din coordinate Recharts
  const x = coordinate?.x || 0;
  const y = coordinate?.y || 0;

  // Lățimea viewportului
  const windowWidth = window.innerWidth;

  // Dacă tooltipul ar ieși din dreapta, îl poziționezi în stânga punctului
  const leftPosition = (x + tooltipWidth + margin > windowWidth)
