...
function CustomTooltip({ active, payload, label, showPercentage,coordinate }) {
 ...
 const tooltipWidth = 550;
  ...
  const topPosition = wouldOverflowBottom
    ? windowHeight - tooltipHeight - margin // Aliniază cu marginea de jos
    : y - 200; // Poziție normală
  ...
  return (
    <div  ref={scrollRef}
      style={{
        position: "fixed", // Schimbat de la absolute la fixed pentru a fi relativ la viewport
        top: `${Math.max(margin, topPosition)}px`, // Asigură-te că nu iese în afara marginilor
