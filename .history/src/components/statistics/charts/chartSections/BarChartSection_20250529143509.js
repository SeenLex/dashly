...
function BarChartSection({ filters }) {
  ...
  // Fetch all tickets to extract project info per priority
  useEffect(() => {
  ...
  const fetchData = async (type, setData) => {
    ...
    if (filters.team_assigned_person)
      params.append("team_assigned_person", filters.team_assigned_person);
    if (filters.team_created_by)
      params.append("team_created_by", filters.team_created_by);
    ...
    try {
      const res = await fetch(
        `http://localhost/api/tickets.php?${params.toString()}`
      );
  ...
  // Construim priorityData cu lista proiectelor aferente fiecarei prioritati
  const priorityDataWithProjects = priorityData.map((priorityItem) => {
    // extrage proiectele din toate ticketele care au aceeasi prioritate
    const projects = allTickets
      .filter((ticket) => ticket.priority === priorityItem.priority)
        ...
        components={[
          ...
          <>
            <CustomBarChart
              ...
              data={priorityDataWithProjects}
              dataKey="count"
              categoryKey="priority"
              // passam tooltip custom prin props
              customTooltip={<CustomTooltip />}
            />
            <button
              style={{ marginTop: 8, marginBottom: 24 }}
              onClick={() =>
                exportDataToCsv("priority-count.csv", priorityDataWithProjects)
              }
            >
              Export CSV
            </button>
          ...
          <>
            <CustomBarChart
              ...
              data={priorityData.map((d) => ({
                ...
                perc: (
                  (d.count /
                    priorityData.reduce((acc, cur) => acc + cur.count, 0)) *
                  100
                ).toFixed(1),
              }))}
              dataKey="perc"
          ...
          <>
            <CustomBarChart
              ...
              data={priorityData}
              dataKey="sla_met"
              categoryKey="priority"  customTooltip={<CustomTooltip />}
            />
            <button
              style={{ marginTop: 8, marginBottom: 24 }}
              onClick={() =>
          ...
          <>
            <CustomBarChart
              ...
              dataKey="perc"
              categoryKey="priority"
              perc={true}  customTooltip={<CustomTooltip />}
            />
            <button
              style={{ marginTop: 8, marginBottom: 24 }}
              onClick={() =>
          ...
          <>
            <CustomBarChart
              ...
              data={teamData}
              dataKey="sla_met"
              categoryKey="team"   customTooltip={<CustomTooltip />}
            />
            <button
              style={{ marginTop: 8, marginBottom: 24 }}
          ...
          <>
            ...
            <button
              style={{ marginTop: 8, marginBottom: 24 }}
              onClick={() =>
                exportDataToCsv("project-sla-met.csv", projectData)
              }
            >
              Export CSV
            </button>
