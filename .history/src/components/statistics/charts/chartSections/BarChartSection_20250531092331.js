...
function BarChartSection({ tickets, totalCount }) {
 ...
 useEffect(() => {
  if (tickets) {
    ...
    const ticketsResolutionSLAPercByPriority = getPercentsFromCounted(ticketsResolutionSLANumByPriority, totalCount);

    const ticketsResolutionSLANumByTeam = countByTeamAssigned(tickets, true);
    const ticketsResolutionSLAPercByTeam = getPercentsFromCounted(ticketsResolutionSLANumByTeam, totalCount, "team");

    const ticketsResolutionSLANumByProject = countByProject(tickets, true);
    const ticketsResolutionSLAPercByProject = getPercentsFromCounted(ticketsResolutionSLANumByProject, totalCount, "project");

    const teamSlaStatus = calculateSlaStatus(tickets, "team_assigned_person_name");
    const teamSlaStatusPercent = convertSlaStatusToPercentages(teamSlaStatus);

    const projectSlaStatus = calculateSlaStatus(tickets, "project_name");
