...
function prepareDataWithTicketsByTeam(tickets) {
  const grouped = {};

  tickets.forEach(ticket => {
    const team = ticket.team_assigned_person || "N/A";
    if (!grouped[team]) grouped[team] = { team, count: 0, tickets: [] };
    grouped[team].count += 1;
    grouped[team].tickets.push(ticket);
  });

  return Object.values(grouped);
}useEffect(() => {
  if (tickets) {
    const dataWithTickets = prepareDataWithTicketsByTeam(tickets);
    setResolutionSLANumByTeam(dataWithTickets);
    // alte setări de state...

    // De exemplu pentru procentaje, poți calcula separat
  }
}, [tickets]);
...
setTeamsByCategory(teamsByCategoryMap);
      ...
      setSlaStatusByProjectPercent(projectSlaStatusPercent);
        ...
        setResolutionSLANumByTeam(prepareDataWithTicketsByCategory(tickets, "team_assigned_person"));
  setResolutionSLANumByProject(prepareDataWithTicketsByCategory(tickets, "project"));
  setResolutionSLANumByPriority(prepareDataWithTicketsByCategory(tickets, "priority"));
