...
function BarChartSection({ tickets, totalCount }) {
  ...
  useEffect(() => {
    ...
    tickets.forEach((ticket) => {
      ...
      if (!teamsByCategoryTemp[catValue])
        teamsByCategoryTemp[catValue] = new Set();
    ...
    const dataWithTicketsByTeam = prepareDataWithTicketsByCategory(
      ...
      // Calculate SLA compliance for each team
      const metCount = team.tickets.filter(
        (t) => t.resolution === "Met"
      ).length;
    ...
    const projectSlaStatus = calculateSlaStatus(tickets, "project").map(
      (item) => ({
        ...item,
      ...
      <CustomHorizontalContainer
        components={[
          ...
          <CustomBarChart
            ...
            title="Resolution SLA by team (count and %)"
          ...
          <CustomBarChart
            ...
            title="Team Performance by Projects"
            data={resolutionSLANumByTeam}
            dataKey="count"
            categoryKey="team_assigned_person"
            teamsByCategory={teamsByCategory}
            showBothValues={true}
            showProjects={true} // Enable project display
            totalCount={totalCount}
            slaStatusByTeam={slaStatusByTeam}
            colors={["#4CAF50"]} // Green color for all bars
          />,
          <CustomBarChart
            key="sla-num-project"
