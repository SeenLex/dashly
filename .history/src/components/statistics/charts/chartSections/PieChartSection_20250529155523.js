...
useEffect(() => {
    ...
    const qsStatus = toQueryString({ ...localFilters, type: "status" });
    fetch(`http://localhost/api/tickets_piechart.php?${qsStatus}`)
      .then((res) => res.json())
      .then(setStatusData);

    const qsPriority = toQueryString({ ...localFilters, type: "priority" });
    fetch(`http://localhost/api/tickets_piechart.php?${qsPriority}`)
      .then((res) => res.json())
      .then(setPriorityData);

    const qsTeam = toQueryString({ ...localFilters, type: "team_assigned_person" });
