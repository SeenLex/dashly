...
useEffect(() => {
  fetch("http://localhost/api/tickets.php")
    ...
    .then((data) => {
      ...
      setAllPriorities([...new Set(data.map(t => t.priority).filter(Boolean))]);  // -> priority, nu priority_name
      setAllProjects([...new Set(data.map(t => t.project).filter(Boolean))]);    // aici cheia e project, nu projects
