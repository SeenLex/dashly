...
function ChartSwitcher() {
  ...
  useEffect(() => {
    fetch("http://localhost/api/tickets.php")
      .then((res) => res.json())
      .then((data) => {
        setAllTeams_assigned([
          ...new Set(data.map((t) => t.team_assigned_person).filter(Boolean)),
        ]);
        setAllTeams_created([
          ...new Set(data.map((t) => t.team_created_by).filter(Boolean)),
        ]);
        setAllPriorities([
          ...new Set(data.map((t) => t.priority).filter(Boolean)),
        ]); // -> priority, nu priority_name
        setAllProjects([
          ...new Set(data.map((t) => t.project).filter(Boolean)),
        ]); // aici cheia e project, nu projects
  ...
  useEffect(() => {
    ...
    if (filters.team_assigned_person)
      params.append("assigned_person", filters.team_assigned_person);
    if (filters.team_created_by)
      params.append("created_by", filters.team_created_by);
    ...
    fetch(`http://localhost/tickets-api/tickets.php?${params.toString()}`)
      .then((res) => res.json())
