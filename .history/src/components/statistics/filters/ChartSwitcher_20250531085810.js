    ...
    fetch("http://localhost/api/tickets.php")
      ...
      .then((data) => {
        ...
        setAllTeams_assigned([
          ...new Set(
            data.map((t) => t.team_assigned_person_name).filter(Boolean)
          ),
        ]); 
        setAllTeams_created([
          ...new Set(data.map((t) => t.team_created_by_name).filter(Boolean)),
        ]);
        setAllPriorities([
          ...new Set(data.map((t) => t.priority_name).filter(Boolean)),
        ]);
        setAllProjects([
          ...new Set(data.map((t) => t.project_name).filter(Boolean)),
        ]);
