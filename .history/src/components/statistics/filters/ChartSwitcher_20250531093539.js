...
useEffect(() => {
  fetch("http://localhost/api/teams.php")
    .then(res => res.json())
...
useEffect(() => {
  fetch("http://localhost/api/tickets.php")
    .then((res) => res.json())
    .then((data) => {
      // Obține toate id-urile unice din tickets
      const uniqueAssignedIds = [...new Set(data.map(t => t.team_assigned_person).filter(Boolean))];
      // Folosește teamMap pentru a obține numele echipelor
      const assignedTeams = uniqueAssignedIds.map(id => ({
        id,
        name: teamMap[id] || `Echipa #${id}`
      }));

      setAllTeams_assigned(assignedTeams);

      // La fel pentru team_created_by dacă e necesar
      const uniqueCreatedIds = [...new Set(data.map(t => t.team_created_by).filter(Boolean))];
      const createdTeams = uniqueCreatedIds.map(id => ({
        id,
        name: teamMap[id] || `Echipa #${id}`
      }));
      setAllTeams_created(createdTeams);

      // Similar pentru alte filtre
      setAllPriorities([...new Set(data.map(t => t.priority).filter(Boolean))]);
      setAllProjects([...new Set(data.map(t => t.project).filter(Boolean))]);
      setAllStatuses([...new Set(data.map(t => t.status).filter(Boolean))]);
    })
