...
function Filters({ filters, setFilters, allValues, onData }) {
  const handleSearch = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value) {
        // Mapping specific pentru startDate, endDate
        if (key === "startDate") queryParams.append("dateFrom", value);
        else if (key === "endDate") queryParams.append("dateTo", value);
    ...
    fetch(`http://localhost/api/tickets.php?${queryParams.toString()}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end space-x-4 mt-6">
        <button onClick={handleSearch} className="custom-button custom-button-active">
          Search
        </button>
        <button onClick={clearFilters} className="custom-button custom-button-inactive">
          Clear Filters
        </button>
      </div>
    </div>
