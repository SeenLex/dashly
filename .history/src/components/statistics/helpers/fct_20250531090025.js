...
export function countByTeamAssigned(tickets, slaOnly = false) {
  ...
  tickets.forEach(ticket => {
    if (slaOnly && ticket.sla_resolved !== "YES") return;
...
export function countByPriority(tickets, slaOnly = false) {
  if (!Array.isArray(tickets)) {
    console.error("❌ countByPriority: expected array, got:", tickets);
    return [];
  }
  ...
  tickets.forEach(ticket => {
    if (slaOnly && ticket.sla_resolved !== "YES") return;
    const priority = ticket.priority_name || "Unknown";
    counts[priority] = (counts[priority] || 0) + 1;
  });

  // Transformă într-un array pentru afișare în grafic
