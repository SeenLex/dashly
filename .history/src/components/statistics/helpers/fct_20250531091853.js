...
export function countByTeamAssigned(tickets, SLAResolutionMandatory = false) {
  ...
  const groupedDataNum = tickets.reduce((acc, ticket) => {
    if ("team_assigned_person" in ticket) {
      const team = ticket.team_assigned_person;
...
export function countByPriority(data, SLAResolutionMandatory = false) {
  ...
  // Dacă vrem doar tichetele rezolvate SLA:
  if (SLAResolutionMandatory) {
    data = data.filter(ticket => getSLAResolution(ticket) === true);
  }

  // Construim un obiect { "High": 3, "Medium": 1, ... }
  const groupedDataNum = data.reduce((acc, ticket) => {
    // Folosim PROPRIETATEA corectă: priority_name
    const key = ticket.priority_name || "Unknown";
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  // Transformăm în array de obiecte pentru grafic:
  const counted = Object.entries(groupedDataNum).map(([priority, count]) => ({
    priority,
    count
  }));

  // (Opțional) sortăm după o ordine de importanță:
...
export function countByProject(tickets, slaOnly = false) {
  ...
  tickets.forEach((ticket) => {
    if (!slaOnly || (ticket.closed_date && ticket.duration_hours)) {
      const project = ticket.project_name || "Unassigned";
...
export function countByTeamAssigned(tickets, slaOnly = false) {
  const counts = {};

  tickets.forEach((ticket) => {
    if (!slaOnly || (ticket.closed_date && ticket.duration_hours)) {
      const team = ticket.team_assigned_person_name || "Unassigned";
      counts[team] = (counts[team] || 0) + 1;
    }
  });

  return Object.entries(counts).map(([team, count]) => ({
    team,
    count,
  }));
}
