...
export default function SuperAdminPanel() {
  ...
  useEffect(() => {
    fetch('http://localhost/tickets-api/get_admins.php')
      .then((res) => res.json())
      .then((data) => setAdmins(data))
      .catch((err) => console.error('Eroare admins:', err));
  }, []);
  useEffect(() => {
    if (viewProjectId) {
      fetch(
        `http://localhost/tickets-api/get_teams_by_project.php?id_project=${viewProjectId}`
      )
        .then((res) => res.json())
  ...
  useEffect(() => {
    if (viewTeamId) {
      fetch(
        `http://localhost/tickets-api/get_users_by_team.php?id_team=${viewTeamId}`
      )
        .then((res) => res.json())
  ...
  const assignAdminToProject = () => {
    if (!selectedProject || !selectedAdmin)
      return alert('Selectați un proiect și un administrator.');
  ...
  const assignProjectToTeam = () => {
    if (!selectedProject || !selectedTeam)
      return alert('Selectați un proiect și o echipă.');
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-slate-200 dark:border-gray-700 p-8 hover:shadow-2xl transition-all duration-300">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {projects.map((proj) => (
                  <option key={proj.id_project} value={proj.id_project}>
                    {proj.provider}
                  </option>
                ))}
              </select>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-12">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-slate-200 dark:border-gray-700 p-8 hover:shadow-2xl transition-all duration-300">
            ...
            <div className="space-y-4">
              <select
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors duration-200 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                value={selectedProject}
                onChange={(e) => setSelectedProject(e.target.value)}
              >
              ...
              <button
                className="w-full bg-gradient-to-r from-purple-500 to-purple-600 text-white px-6 py-3 rounded-xl hover:from-purple-600 hover:to-purple-700 transform hover:scale-105 transition-all duration-200 font-semibold shadow-lg"
                onClick={assignAdminToProject}
              >
                Asignează
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-slate-200 dark:border-gray-700 p-8 hover:shadow-2xl transition-all duration-300">
            ...
            <div className="space-y-4">
              <select
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                value={selectedProject}
                onChange={(e) => setSelectedProject(e.target.value)}
              >
                <option value="">Proiect</option>
              ...
              <button
                className="w-full bg-gradient-to-r from-indigo-500 to-indigo-600 text-white px-6 py-3 rounded-xl hover:from-indigo-600 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 font-semibold shadow-lg"
                onClick={assignProjectToTeam}
              >
                Asignează
              </button>
            </div>
